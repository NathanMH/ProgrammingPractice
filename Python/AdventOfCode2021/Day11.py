path = "/home/natha/Documents/python/AdventOfCode/assets/Day11.txt"
#!/usr/bin/python3
...
def count_neighbors(state, j, i):
    neighbors = [
        state[y][x] == "#"
        if y >= 0 and y < h and x >= 0 and x < w and not (y == j and x == i)
        else False
        ...
        for j in range(h):
            for i in range(w):
                if next_state[j][i] == ".":
                    ...
                    state[j][i] = (
                        "L" if count_function(next_state, j, i) >= tol else "#"
                    )
                else:
...
def count_neighbors_new(next_state, j, i):
    ...
    for direction in directions:
        ...
        while True:
            ...
            if not (y >= 0 and y < h and x >= 0 and x < w) or next_state[y][x] == "L":
                break
            if next_state[y][x] == "#":
                count += 1
                break
